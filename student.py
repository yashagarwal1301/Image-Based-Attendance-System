...
class Student:
    ...
    # This part is image labels setting start
        # first header image
        img = Image.open(
            r"D:\Final Project\Images_GUI\banner.jpg")
        ...
        # backgorund image
        bg1 = Image.open(
            r"D:\Final Project\Images_GUI\bg3.jpg")
        ...
        dep_combo["values"] = ("Select Department", "CSE","IT", "ECE", "ME", "CV")
        dep_combo.current(0)
        ...
        cou_combo["values"] = ("Select Course", "C1", "C2", "C3", "C4", "C5")
        cou_combo.current(0)
        ...
        # create table
        self.student_table = ttk.Treeview(table_frame, column=("ID", "Name", "Dep", "Course", "Year", "Sem", "Div", "Gender", "DOB", "Mob-No", "Address", "USN", "Email", "Teacher", "Photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        if self.var_dep.get() == "Select Department" or self.var_course.get() == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester" or self.var_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(username='root', password='root', host='localhost', database='face_recognition', port=3306)
                    mycursor = conn.cursor()
                    mycursor.execute("update student set Student_ID=%s,Department=%s,Name=%s,Course=%s,Year=%s,Semester=%s,Division=%s,Gender=%s,DOB=%s,Mobile_No=%s,Address=%s,USN=%s,Email=%s,Teacher_Name=%s,PhotoSample=%s where Student_ID=%s", (
                        self.var_std_id.get(),
                        self.var_dep.get(),
                        self.var_std_name.get(),
                        self.var_course.get(),
    ...
    def search_data(self):
        if self.var_search.get() == "" or self.var_searchTX.get() == "Select":
            ...
            try:
                ...
                sql = "SELECT Student_ID,Name,Department,Course,Year,Semester,Division,Gender,DOB,Mobile_No,Address,USN,Email,Teacher_Name,PhotoSample FROM student where USN='" + \
                    str(self.var_search.get()) + "'"
                my_cursor.execute(sql)
                # my_cursor.execute("select * from student where USN= " +str(self.var_search.get())+" "+str(self.var_searchTX.get())+"")
    ...
    def generate_dataset(self):
        if self.var_dep.get() == "Select Department" or self.var_course.get == "Select Course" or self.var_year.get() == "Select Year" or self.var_semester.get() == "Select Semester" or self.var_std_id.get() == "" or self.var_std_name.get() == "" or self.var_div.get() == "" or self.var_roll.get() == "" or self.var_gender.get() == "" or self.var_dob.get() == "" or self.var_email.get() == "" or self.var_mob.get() == "" or self.var_address.get() == "" or self.var_teacher.get() == "":
            ...
            try:
                ...
                mycursor.execute("update student set Name=%s,Department=%s,Course=%s,Year=%s,Semester=%s,Division=%s,Gender=%s,DOB=%s,Mobile_No=%s,Address=%s,USN=%s,Email=%s,Teacher_Name=%s,PhotoSample=%s where Student_ID=%s", (
                    self.var_std_name.get(),
                    self.var_dep.get(),
                    self.var_course.get(),
                    self.var_year.get(),
                ...
                while True:
                    ...
                    if face_croped(my_frame) is not None:
                        ...
                        file_path = f"data_img/{usn}.{str(self.var_std_id.get())}.{str(img_id)}.jpg"
                        print(file_path)
                        cv2.imwrite(file_path, face)
                        cv2.putText(face, str(usn), (50, 50),cv2.FONT_HERSHEY_COMPLEX, 2, (0, 255, 0), 2)
                        cv2.imshow("Capture Images", face)

                    if cv2.waitKey(1) == 9 or int(img_id) == 150:           #number of images to capture per person for training
                        break
